...
async function ensureLowStockAlert(db, { productId, productName, stock, limit }) {
  const pid = toInt(productId);
  if (!pid) return { created: false, reason: "INVALID_PRODUCT" };
  ...
  if (s > l) return { created: false, reason: "ABOVE_LIMIT" };
  ...
  const existing = await db.alert.findFirst({
    where: {
      ...
      OR: [
        ...
        ...(name ? [{ Message: { contains: `Low stock for product: ${name}` } }] : []),
      ],
    },
    select: { AlertID: true },
  });

  if (existing?.AlertID) return { created: false, reason: "EXISTS", alertId: existing.AlertID };

  const Message = buildLowStockMessage({ productId: pid, productName, stock: s, limit: l });
...
module.exports = { ensureLowStockAlert, buildLowStockMessage };
